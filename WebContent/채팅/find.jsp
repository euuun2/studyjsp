	...
	<script type="text/javascript">
		function findFunction() {
			...
			$.ajax({
				...
				data: {userID: userID},
				success: function(result) {			// 반환 값이 인자 값으로
					if(result == 0){
					...
					$('#checkModal').modal("show");			// 모달 창으로 띄움
				}
			});
		}
		...
		function getFriend(findID) {
			$('#friendResult').html('<thead>'+
					...
					'<tbody>'+
					'<tr>'+
					'<td style="text-align: center;"><h3>' + findID + '</h3><a href="chat.jsp?newID=' + encodeURIComponent(findID)+ '" class="btn btn-primary pull-right">' + '메시지 보내기</a></td>'+
																											// 이게 나중에 친구초대 버튼 역할	
					'</tr>'+
					'</tbody>');
		}
	...
	<nav class="navbar navbar-default">
		...
		<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
			<ul class="nav navbar-nav">
				...
				<li><a href="chattingRoom.jsp">메시지함</a>				
				</li>
			</ul>
